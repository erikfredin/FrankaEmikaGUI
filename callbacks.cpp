...
//        std::cout<<"Raw Analog Votages "<< t << ": "<< inputAnalogVoltages[t]<<std::endl;
        ...
        if(currentStreaming)
        {
            ...
            rawADinput_currentfeed[t] = inputAnalogVoltages[t];
        }
...
//            double abs_robotpos[3] = {robot_x, robot_y, robot_z};
            ...
            Eigen::Vector4d pos_robot = transT2R*pos_cmd;
            //Origin point P_0 (//read robot status ReadFrankaPoseStatus();),  desired point P_d = P_0+delta(x,y,z)
            double abs_robotpos[3] = {pos_robot(0), pos_robot(1), pos_robot(2)};
...
//        connect(socket, SIGNAL(readyRead()), this, SLOT(processPendingDatagrams()),Qt::QueuedConnection);
        ...
        if(capsuleOutplane)
//        if(false) //move robot up/down to search for the capsule
        {
            std::cout<<"capsule out plane motion........"<<std::endl;
//            double maxSearchRange = 4*0.001; //unit:meter
            double maxSearchRangeUp = 3*0.01; //unit:meter
            double maxSearchRangeDown = 3*0.01; //unit:meter
            double deltaMove = 2*0.001; //unit:meter
            if (USrobotUpward==true)
            {
                if(USrobotVelticalMove<maxSearchRangeUp)
                {
                    double Relativemoiton[3] = {0.0, 0.0, deltaMove};
// Uncomment below to active Franka robot control !//
                    ReadFrankaPoseStatus();
...
//                   // FrankaRelativecartmotion(Relativemoiton);
            ...
            {
                if(USrobotVelticalMove>(-maxSearchRangeDown))
                {
                    double Relativemoiton[3] = {0.0, 0.0, -deltaMove};
// Uncomment below to active Franka robot control !//
                    ReadFrankaPoseStatus();
...
//            double delta_image[3] = {-(USimage_pos(0)-USimageCenter(0)), -(USimage_pos(1)-USimageCenter(1)),-(USimage_pos(2)-USimageCenter(2))};
            double delta_image[3] = {-(USimage_pos(0)-USimageCenter(0))*0.001*inplaneSpeed, 0.0, 0.0};
